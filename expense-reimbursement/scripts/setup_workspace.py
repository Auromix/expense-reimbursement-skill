...
用法:
  python3 setup_workspace.py --source <原始材料文件夹> --out <工作区根目录> [--template <报销模板.xlsx>]

行为:
  1. 在 <工作区根目录> 下创建 10 个子文件夹(以 SUBFOLDERS 为准):
     有票 无票 辅助材料 待核实 替票 额外说明 过程文件 备份 补充材料 无关
...
# 替票:   客户(可选)提供的、平时其他消费的正规发票池, 在金额上大致冲抵"无票"项给财务交差。
#         替票与无票不是一一对应的真票: 替票留在本目录、不转入 有票/, 无票也不改判为有票;
#         不计入台账金额, 只在 过程文件/替票池.csv 做替票池总额 vs 无票总额的粗略覆盖对比。
