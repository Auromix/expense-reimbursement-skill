...
### 第 9 步：替票（按需，客户可给可不给）
有些"无票"项是真实消费但**拿不到正式发票**(如公司租房水电费、零星个人代付)。客户**可以选择**提供平时其他消费的正规发票放进 `替票/`，用于在**金额上大致凑齐/冲抵**这些无票项、给财务附上发票材料——也可以**不提供**。

**最关键的一条：替票与无票并不是一一对应的真票，只是金额上接近。** 所以：
- **替票绝不转入 `有票/`；无票项也绝不改判为「有票」**——无票始终按无票管理(它才是这笔真实消费的记录)。
- 替票留在 `替票/`，作为另外附给财务的发票**单独存放**，**不进台账费用行、不计入报销金额**(否则会与无票项重复计钱)。
- 只在 `过程文件/替票池.csv` 记一份替票清单与**替票池总额**，并和**无票总额**做个粗略对比(够不够覆盖)；**不做 1:1 映射、不改桶、不改 ledger 金额**。

**只要本次存在「无票」项，就在沟通里提醒客户一句**：可**选择性**把替票放进 `替票/`(列出无票项与合计金额，并说明"可给可不给")。不要替客户硬凑、不要假设客户一定会补。
