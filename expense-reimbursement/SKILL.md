...
- 各桶文件计数、额外说明清单。
**交付前必跑：Windows 编码收尾**（财务多在 Windows 打开，无 BOM 的 UTF-8 中文会乱码）：
```bash
python3 scripts/finalize_encoding.py <工作区根目录>
```
它把工作区里所有 `.txt/.md/.csv` 统一成 **UTF-8 带 BOM**(Windows 记事本/Excel 不乱码)、并把文件名规范成 **NFC**(mac 的 NFD 名在 Windows 易显示异常)；`.json` 保持纯 UTF-8、`.xlsx`/二进制不动。
> 自己手写 `审阅报告.md`、`额外说明/*.txt` 时也要存成 UTF-8 带 BOM；跑一遍本脚本即可兜底。`make_default_table.py` 生成的 md 已自带 BOM；台账 csv 用 utf-8-sig。
